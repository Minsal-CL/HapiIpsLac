...
import ca.uhn.fhir.jpa.ips.strategy.LacBundleDdccIpsGenerationStrategy;
import ca.uhn.fhir.jpa.ips.strategy.LacBundleIpsGenerationStrategy;
...
public class IpsGeneratorSvcImpl implements IIpsGeneratorSvc {
	...
	/**
	 * Constructor con una sola estrategia (compatibilidad con código legado)
	 */
		// Constructor con una sola estrategia
	// public IpsGeneratorSvcImpl(FhirContext theFhirContext, DaoRegistry theDaoRegistry) {
	// 	this(theFhirContext, List.of(
	// 		createDefaultStrategy(theFhirContext, theDaoRegistry),
	// 		createLacStrategy(theFhirContext, theDaoRegistry)
	// 	));
	// }


	// Constructor con múltiples estrategias
	public IpsGeneratorSvcImpl(FhirContext theFhirContext, DaoRegistry theDaoRegistry) {
		this.myFhirContext = theFhirContext;

		// Crear e inyectar dependencias
		DefaultJpaIpsGenerationStrategy defaultStrategy = new DefaultJpaIpsGenerationStrategy();
		defaultStrategy.setFhirContext(theFhirContext);
		defaultStrategy.setDaoRegistry(theDaoRegistry);
		defaultStrategy.initialize();

		LacBundleDdccIpsGenerationStrategy ddccStrategy = new LacBundleDdccIpsGenerationStrategy();
		ddccStrategy.setFhirContext(theFhirContext);
		ddccStrategy.setDaoRegistry(theDaoRegistry);
		ddccStrategy.initialize();

		LacBundleIpsGenerationStrategy lacStrategy = new LacBundleIpsGenerationStrategy();
		lacStrategy.setFhirContext(theFhirContext);
		lacStrategy.setDaoRegistry(theDaoRegistry);
		lacStrategy.initialize();

		this.myGenerationStrategies = List.of(
			defaultStrategy, 
			ddccStrategy,
			lacStrategy
		);
	...
	IIpsGenerationStrategy selectGenerationStrategy(@Nullable String theRequestedProfile) {
			if (isBlank(theRequestedProfile)) {
				return myGenerationStrategies.get(0); // default
			}
			...
			System.out.println("No matching strategy found for profile: " + theRequestedProfile);
			return myGenerationStrategies.get(0);
		}
	...
	private void addSection(
		...
		for (IBaseResource next : theResourcesToInclude.getResources()) {
			...
			//String fullUrl = theStrategy.getBaseUrl() + "/" + myFhirContext.getResourceType(next) + "/" + next.getIdElement().getIdPart();
			// IdType fullUrl = new IdType(theStrategy.getBaseUrl(),myFhirContext.getResourceType(next),next.getIdElement().getIdPart());
			// String newUrl = new String(next.getIdElement().toUnqualifiedVersionless().getValue());

			// // Construye la URL completa utilizando la base del servidor
			// String[] parts = newUrl.split("/");
			// String result = parts[1];  // Obtiene "con-02"
			// IdType fullUrl = new IdType(theStrategy.getBaseUrl(), myFhirContext.getResourceType(next), result);
			//System.out.println(new IdType(next.getIdElement().getValue()));
			// Agrega la URL completa al entry de la sección del Composition
